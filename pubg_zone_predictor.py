...
# Regular map image dimensions (all maps are 3072x3072)
map_dimensions = {
    "Erangel": (3072, 3072),
    "Miramar": (3072, 3072),
    "Sanhok": (3072, 3072),
...
# Load heatmaps as color images (now assumed to be 3072x3072 and perfectly aligned)
...
try:
    if os.path.exists("erangel_heatmap.jpg"):
        img = Image.open("erangel_heatmap.jpg").convert("RGB")
        erangel_heatmap = np.array(img)
    if os.path.exists("miramar_heatmap.jpg"):
        img = Image.open("miramar_heatmap.jpg").convert("RGB")
        miramar_heatmap = np.array(img)
    if os.path.exists("vikendi_heatmap.jpg"):
        img = Image.open("vikendi_heatmap.jpg").convert("RGB")
        vikendi_heatmap = np.array(img)
    if os.path.exists("taego_heatmap.jpg"):
        img = Image.open("taego_heatmap.jpg").convert("RGB")
        taego_heatmap = np.array(img)
...
def compute_average_danger(center, radius, map_name, map_width, map_height):
    """
    Samples the candidate zone at a 5-pixel resolution and returns the average "danger" score.
    Danger is computed per pixel as:
         danger = R / (R + G + B)
    This yields 1.0 for pure red and lower values for cooler colors.
    Since the heatmap and regular map are both 3072x3072, the conversion ratio is 1.
    """
    if map_name == "Erangel":
        ...
        for dy in range(-rr, rr+1, 5):
            if dx*dx + dy*dy <= rr*rr:
                ...
                if 0 <= px < map_width and 0 <= py < map_height:
                    ...
                    s = float(pixel[0] + pixel[1] + pixel[2])
                    # Avoid division by zero; if s==0, assume danger 0.
                    danger = (pixel[0] / s) if s > 0 else 0.0
                    total_danger += danger
                    sample_count += 1
    if sample_count == 0:
        return 1.0
    return total_danger / sample_count
...
with st.sidebar:
    ...
    if st.button("Predict Next Zone"):
        if any(st.session_state.zones):
            # Load regular map image (3072x3072) and convert to array
            ...
            if current_phase < 4:
                ...
                # For Z4 and later, sample 100 candidates and choose the one with the lowest average danger.
                best_avg_danger = float('inf')
                for attempt in range(100):
                    ...
                    if not is_zone_on_land(new_center, new_radius, img_array):
                        continue
                    avg_danger = compute_average_danger(new_center, new_radius, map_name, width, height)
                    if avg_danger < best_avg_danger:
                        best_avg_danger = avg_danger
                        best_candidate = (new_center, new_radius)
